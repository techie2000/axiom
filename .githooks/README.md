...
If `frontend/public/locales/en/common.json` is part of the staged set, the hook runs
`frontend/scripts/alias-common-i18n-values.mjs`.

The script finds duplicate plain-string values outside `common.*` and replaces them with
`$t(common.<key>)` aliases when an exact canonical value already exists in `common.*`.
If replacements are made, the locale file is automatically re-staged.

This helps avoid duplicate labels like `"Country"` and `"City"` being repeated in multiple sections.

If `node` is unavailable, the commit is blocked for staged locale changes.
...
| `node` | i18n duplicate aliasing in `en/common.json` (pre-commit, staged locale changes) | Install Node.js and run `node --version` |
