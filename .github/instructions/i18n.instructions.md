...
### Reuse canonical button keys (Required)

Do not create page-local duplicates for standard header/action labels when canonical keys already exist.

Use these canonical keys for new pages:
...
// ❌ BAD (duplicate literals)
"provisionalLei": {
  "controls": {
    "columns": "⚙️ Columns ({{count}})",
    "displayNames": "🏷️ Display: Names",
    "displayCodes": "🏷️ Display: Codes"
  }
}
```
