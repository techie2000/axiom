...
WITH keep AS (
    ...
    ORDER BY
        elf_code ASC,
        CASE WHEN COALESCE(language_code, '') = 'en' THEN 0 ELSE 1 END, -- noqa: RF04
        updated_at DESC,
        created_at DESC
)
...
WHERE
    t.elf_code IS NOT NULL
    AND BTRIM(t.elf_code) <> ''
    AND NOT EXISTS (
        SELECT 1 AS col1 FROM keep AS k
        WHERE k.id = t.id
    );
...
WHERE
    t.role_code IS NOT NULL
    AND BTRIM(t.role_code) <> ''
    AND NOT EXISTS (
        SELECT 1 AS col1 FROM keep_role AS k
        WHERE k.id = t.id
    );
