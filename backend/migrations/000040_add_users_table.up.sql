...
COMMENT ON COLUMN users.username IS 'Unique display username (alphanumeric + underscores).';
COMMENT ON COLUMN users.password_hash IS 'bcrypt hash of the user password. Never stored in plain text.';
COMMENT ON COLUMN users.full_name IS 'Human-readable full name of the user.';
COMMENT ON COLUMN users.role IS 'Authorisation role: admin (full access) or user (read/restricted access).';
COMMENT ON COLUMN users.status IS
'Provisioning lifecycle: pending (awaiting admin approval), active (can log in), inactive (deactivated).';
COMMENT ON COLUMN users.approved_by IS 'UUID of the admin user who approved this account. NULL while pending.';
COMMENT ON COLUMN users.approved_at IS 'Timestamp when the account was approved. NULL while pending.';
COMMENT ON COLUMN users.is_bootstrap IS
...
COMMENT ON COLUMN users.deleted_at IS 'Soft-delete timestamp. NULL when the record is active.';
