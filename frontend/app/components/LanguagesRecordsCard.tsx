...
export default function LanguagesRecordsCard() {
  ...
  useEffect(() => {
    const fetchRecordCount = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/v1/languages`, { cache: 'no-store' })
        ...
        console.error('Failed to fetch languages count:', error)
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <Link href="/languages" className="group bg-white border-2 border-gray-200 dark:bg-white/5 dark:border-white/10 backdrop-blur-sm rounded-lg shadow-lg hover:shadow-xl transition-all p-6 hover:border-green-500 dark:hover:border-green-400 min-h-[240px] flex flex-col">
