-- Migration 011: Skip audit records for no-op UPDATE operations
-- Rationale: UPSERT operations execute UPDATE even when values are identical
-- This results in audit records with empty changed_fields arrays
-- Solution: Only create audit record if fields actually changed

\echo 'Updating function: reference.audit_countries_changes() to skip no-op updates'
...
\echo 'Audit function updated to skip no-op updates'
\echo 'Example: UPSERT with identical values will not create audit record'
